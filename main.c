...
//get Format optionas
    if(ISCHECKED(FORMAT_128x128)){ ALLOW_128x128 = 1; }else{ ALLOW_128x128 = 0; }
    if(ISCHECKED(FORMAT_128x64)){  ALLOW_128x64  = 1; }else{ ALLOW_128x64  = 0; }
    if(ISCHECKED(FORMAT_64x64)){   ALLOW_64x64   = 1; }else{ ALLOW_64x64   = 0; }
    if(ISCHECKED(FORMAT_64x32)){   ALLOW_64x32   = 1; }else{ ALLOW_64x32   = 0; }
    if( (ALLOW_128x128+ALLOW_128x64+ALLOW_64x64+ALLOW_64x32) == 0)
    {
        SMS("You Must Select At Least 1 Format Option!!!!");
        EnableWindow(B_go,1);
        return;
    }     
    
    len = 5*1024*1024; //5Mb, .png skin files are < 1Mb, but 5MB limit is more than enough.
    SetLimit(len,ALLOW_128x128,ALLOW_128x64,ALLOW_64x64, ALLOW_64x32);    
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        case WM_CREATE:
        {
             ...
             CreateWindow("BUTTON","Description",WS_CHILD|WS_VISIBLE|BS_GROUPBOX,2,57,217,50,hwnd,NULL,ins,NULL);
             SKPdescription= CreateWindowEx(WS_EX_CLIENTEDGE,"edit","",WS_CHILD|WS_VISIBLE,11,77,200,22,hwnd,NULL,ins,NULL);
    		 OldDescEditProc=(WNDPROC)SetWindowLong(SKPdescription, GWL_WNDPROC, (LPARAM)DescEditProc);
         
//geometry            
             CreateWindow("BUTTON","Geometry",WS_CHILD|WS_VISIBLE|BS_GROUPBOX,222,2,117,105,hwnd,NULL,ins,NULL);
             GEOMETRY_NONE = CreateWindow("BUTTON","None",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,229,21,100,19,hwnd,NULL,ins,NULL);	
             GEOMETRY_HEROIS = CreateWindow("BUTTON","Herois",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,229,42,100,19,hwnd,NULL,ins,NULL);	
             GEOMETRY_CUSTOM = CreateWindow("BUTTON","Custom",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,229,63,100,19,hwnd,NULL,ins,NULL);	
             GEOMETRY_CUSTOMSLIM = CreateWindow("BUTTON","CustomSlim",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,229,84,100,19,hwnd,NULL,ins,NULL);	
//format           
             CreateWindow("BUTTON","Format",WS_CHILD|WS_VISIBLE|BS_GROUPBOX,342,2,107,105,hwnd,NULL,ins,NULL);
             FORMAT_128x128 = CreateWindow("BUTTON","128 x 128",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,349,21,90,19,hwnd,NULL,ins,NULL);	
             FORMAT_128x64 = CreateWindow("BUTTON","128 x 64",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,349,42,90,19,hwnd,NULL,ins,NULL);	
             FORMAT_64x64 = CreateWindow("BUTTON","64 x 64",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,349,63,90,19,hwnd,NULL,ins,NULL);	
             FORMAT_64x32 = CreateWindow("BUTTON","64 x 32",WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX,349,84,90,19,hwnd,NULL,ins,NULL);	
//hStatus              
             CreateWindow("BUTTON","STATUS",WS_CHILD|WS_VISIBLE|BS_GROUPBOX|BS_CENTER,2,110,392,47,hwnd,NULL,ins,NULL);
             hStatus[0] = CreateWindow("BUTTON","ZIP",WS_CHILD|WS_VISIBLE|WS_DISABLED,8,129,45,22,hwnd,NULL,ins,NULL);
             hStatus[1] = CreateWindow("BUTTON",".png",WS_CHILD|WS_VISIBLE|WS_DISABLED,55,129,45,22,hwnd,NULL,ins,NULL);
             hStatus[2] = CreateWindow("BUTTON","GEO",WS_CHILD|WS_VISIBLE|WS_DISABLED,102,129,45,22,hwnd,NULL,ins,NULL);
             hStatus[3] = CreateWindow("BUTTON",".lang",WS_CHILD|WS_VISIBLE|WS_DISABLED,149,129,45,22,hwnd,NULL,ins,NULL);
             hStatus[4] = CreateWindow("BUTTON",".json",WS_CHILD|WS_VISIBLE|WS_DISABLED,196,129,45,22,hwnd,NULL,ins,NULL);
             hStatus[5] = CreateWindow("BUTTON","GEO",WS_CHILD|WS_VISIBLE|WS_DISABLED,243,129,45,22,hwnd,NULL,ins,NULL);
             hStatus[6] = CreateWindow("BUTTON",".json",WS_CHILD|WS_VISIBLE|WS_DISABLED,290,129,45,22,hwnd,NULL,ins,NULL);
             hStatus[7] = CreateWindow("BUTTON","End",WS_CHILD|WS_VISIBLE|WS_DISABLED,337,129,45,22,hwnd,NULL,ins,NULL);

             B_go = CreateWindow("BUTTON","START",WS_CHILD|WS_VISIBLE|WS_DISABLED,396,109,54,47,hwnd,(HMENU)1111,ins,NULL);
             
             SNDMSG(GEOMETRY_NONE,BM_SETCHECK,(WPARAM)BST_CHECKED,(LPARAM)0); 
             SNDMSG(FORMAT_128x128,BM_SETCHECK,(WPARAM)BST_CHECKED,(LPARAM)0); 
             SNDMSG(FORMAT_128x64,BM_SETCHECK,(WPARAM)BST_CHECKED,(LPARAM)0); 
             SNDMSG(FORMAT_64x64,BM_SETCHECK,(WPARAM)BST_CHECKED,(LPARAM)0); 
             SNDMSG(FORMAT_64x32,BM_SETCHECK,(WPARAM)BST_CHECKED,(LPARAM)0); 
             CenterOnScreen();
...
int WINAPI WinMain (HINSTANCE hThisInstance, HINSTANCE hPrevInstance, LPSTR lpszArgument, int nFunsterStil)
{
    ...
    457,190,HWND_DESKTOP,NULL,hThisInstance,NULL );
