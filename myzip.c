...
DWORD LimitSize = 0xFFFFFFFF;
UCHAR FORMAT[4] ={ 1, 1, 1, 1 };

void SetLimit(DWORD maxBuffer, UCHAR f128x128, UCHAR f128x64, UCHAR f64x64, UCHAR f64x32)
{
     if(maxBuffer > 0){ LimitSize=maxBuffer; }else{ LimitSize = 0xFFFFFFFF; } 
     FORMAT[0] = f128x128;
     FORMAT[1] = f128x64;
     FORMAT[2] = f64x64;
     FORMAT[3] = f64x32;
...
{
    UCHAR sWidth  = buffer[19];
    UCHAR sHeight = buffer[23];
    
    if(FORMAT[0] == 1) // 128 x 128
    {
        if( (sWidth == 128) &&  (sHeight == 128 ) )
          return YES;       
    }
    if(FORMAT[1] == 1) // 128 x 64
    {
        if( (sWidth == 128) &&  (sHeight == 64 ) )
          return YES;       
    }
    if(FORMAT[2] == 1) // 64 x 64
    {
        if( (sWidth == 64) &&  (sHeight == 64 ) )
          return YES;       
    }
    if(FORMAT[3] == 1) // 64 x 32
    {
        if( (sWidth == 64) &&  (sHeight == 32 ) )
          return YES;       
    } 
return NO;     
}
...
UCHAR FileToZip(ZIP *Z,UCHAR *path,UCHAR *name)
{
    ...
    if(dwFileSize >= LimitSize)
    {
        strcpy(Z->err,"LIMIT SIZE!!!\0");                                  
        CloseHandle(hFile);
        Z->flag=YES;
        return NO;
    }            
